...
layui.define(function(exports){
  ...
  Class.prototype.view = function(){
    ...
    config.prev = 'prev' in config ? config.prev : 'Prev'; //上一页文本
    config.next = 'next' in config ? config.next : 'Next'; //下一页文本
    ...
    ,views = {
      ...
      prev: function(){
        return config.prev 
          ? '<a href="javascript:;" class="layui-laypage-prev maccms-icon'+ (config.curr == 1 ? (' ' + DISABLED) : '') +'" data-page="'+ (config.curr - 1) +'">'+ config.prev +'</a>'
        : '';
      }()
      ...
      ,page: function(){
        ...
        if(index > 1 && config.first !== false && groups !== 0){
          pager.push('<a href="javascript:;" class="layui-laypage-first maccms-icon" data-page="1"  title="First page">'+ (config.first || 1) +'</a>');
        }
      ...
      ,next: function(){
        return config.next 
          ? '<a href="javascript:;" class="layui-laypage-next maccms-icon'+ (config.curr == config.pages ? (' ' + DISABLED) : '') +'" data-page="'+ (config.curr + 1) +'">'+ config.next +'</a>'
        : '';
      }()
      
      //数据总数
      ,count: '<span class="layui-laypage-count">Total '+ config.count +' items</span>'
      ...
      ,skip: function(){
        return ['<span class="layui-laypage-skip">Go to page'
          ...
          ,'&#x9875;<button type="button" class="layui-laypage-btn">Confirm</button>'
        ,'</span>'].join('');
      }()
    };
