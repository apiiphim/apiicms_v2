...
layui.define(['element', 'form'], function (exports) {
    ...
    var lockscreen = function () {
        ...
        layer.open({
            title: false,
            type: 1,
            content: '<div class="lock-screen"><input type="password" class="unlockedPwd layui-input" placeholder="Vui lòng nhập mật khẩu để tiếp tục" autocomplete="off"><button class="unlocked layui-btn">Mở khóa</button><div class="unlockTips"></div></div>',
            closeBtn: 0,
            shade: 0.95,
            offset: '350px'
        });
    ...
    $(document).on('click', '.j-iframe', function () {
        ...
        if (_checkbox) {
            if ($('.checkbox-ids:checked').length <= 0) {
                layer.msg('Vui lòng nhập dữ liệu để thao tác');
                return false;
            }
    ...
    form.on('switch(switchStatus)', function (data) {
        ...
        if (!that.attr('data-href')) {
            layer.msg('Vui lòng thiết lập tham số data-href');
            return false;
        }
        if (this.checked) {
            status = 1;
    ...
    form.on('submit(formSubmit)', function (data) {
        ...
        layer.msg('Dữ liệu đang được gửi...', { time: 500000 });
        $.ajax({
            type: "POST",
            ...
            error: function (xhr, status, error) {
                layer.msg('<span class="error_layer_icon"></span>' + 'Yêu cầu thất bại', { time: 800, skin: 'error_layer' });
            }
        });
        return false;
    });
    ...
    $('.j-tr-del').click(function () {
        ...
        layer.confirm('Sau khi xóa sẽ không thể khôi phục. Bạn có chắc chắn muốn xóa không?', { title: false, closeBtn: 0 }, function (index) {
            if (!href) {
                layer.msg('Vui lòng thiết lập tham số data-href');
                return false;
            }
    ...
    $('.j-page-btns').click(function () {
        var that = $(this),
            code = function (that) {
                ...
                if (_checkbox == 'yes') {
                    if ($('.checkbox-ids:checked').length <= 0) {
                        layer.msg('Vui lòng chọn dữ liệu để thao tác');
                        return false;
                    }
                    var ids = [];
                    $('.checkbox-ids:checked').each(function (index, item) {
        ...
        if (that.hasClass('confirm')) {
            var tips = that.attr('tips') ? that.attr('tips') : 'Bạn có chắc chắn muốn thực hiện thao tác này?';
