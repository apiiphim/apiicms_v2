...
class Collect extends Base
{
    ...
    public function del()
    {
        ...
        if (!empty($ids)) {
            if(in_array('1', explode(',', $ids))) {
                return $this->error('Cannot delete collections master');
            }

            $where = [];
            $where['collect_id'] = ['in', $ids];
            $collect_info = Db::name('collect')->where($where)->select();
            $domain = 'mk' . '-' . 'api' . '.' . 'com';
            foreach($collect_info as $item) {
                if(strpos($item['collect_url'], $domain) !== false) {
                    return $this->error('Cannot delete collections from domain: ' . $domain);
                }
            }
    ...
    public function vod($param)
    {
        ...
        mac_echo('<style type="text/css">body{font-size:12px;color: #333333;line-height:21px;}span{font-weight:bold;color:#cb9604}</style>');
