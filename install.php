<?php
/*
'软件名称：Phim-CMS 源码库：https://github.com/magicblack
'--------------------------------------------------------
...
// 检测PHP环境
if(version_compare(PHP_VERSION,'5.5.0','<'))  die('PHP version requires >=5.5, please upgrade [PHP version requires >=5.5, please upgrade]');
//超时时间
...
if(is_file('./application/data/install/install.lock')) {
	echo 'If you need to reinstall, please delete [To re install, please remove】 >>> /application/data/install/install.lock';
	exit;
}

if(!is_writable('./runtime')) {
	echo 'Please turn on the read and write permissions of the [runtime] folder]';
	exit;
}
